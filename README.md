...
## ⚙️ 配置提示
- API 代理支持通过 Cloudflare Pages 的 **Functions → Environment variables / Secrets** 配置上游地址，无需改代码：
  - `API_BASE_URL`：单个上游 API 地址（例如 `https://api.tunefree.fun/api.php`）。
  - `API_BASE_URLS`：多个上游 API 地址（逗号或换行分隔），代理会按顺序自动回退。
  - 若两者都未设置，则默认使用 `https://music-api.gdstudio.xyz/api.php`。
...
## ❓ 常见问题解答
- **搜索没有结果怎么办？** 检查浏览器控制台日志，如接口被阻挡可尝试切换数据源，或在 Cloudflare Pages 中更新 `API_BASE_URL` / `API_BASE_URLS` 到可用服务；免费 API 出现波动很常见。
